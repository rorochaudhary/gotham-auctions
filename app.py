...
def root():
    ...
    if request.method == 'GET':
        ...
        query = "SELECT * FROM Listings WHERE listingID IN (SELECT listingID FROM (SELECT L.listingID, CONCAT_WS(' ', L.year, L.make, L.model, F.carFeature) AS car_info FROM Listings L INNER JOIN FeaturesListings FL ON L.listingID = FL.listingID INNER JOIN Features F ON FL.featureID = F.featureID) AS tmp WHERE tmp.car_info LIKE %s) AND userID IS NOT NULL AND expirationDate >= NOW();"
